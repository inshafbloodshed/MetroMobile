// src/components/inventory/RepairBillingPanel.js
...
export function RepairBillingPanel({ user }) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const [jobsData, billedData, workersData] = await Promise.all([
          load(SK.JOBS, []),
          load(SK.BILLED, []),
          load(SK.WORKERS, [])
        ]);
  ...
  const getWorkerName = (username) => {
    const worker = workersArray.find(w => w.username === username);
  ...
  const processBilling = async () => {
    ...
    if (amount <= 0) { 
      toast('⚠️ Please enter a valid bill amount'); 
      return; 
    }
    ...
    try {
      ...
      const updatedJobs = jobsArray.filter(j => j.orderId !== selectedJob.orderId);
      setJobs(updatedJobs);
      await save(SK.JOBS, updatedJobs);

      // Add to billed records
      const updatedBilled = [...billedArray, billRecord];
      setBilled(updatedBilled);
      await save(SK.BILLED, updatedBilled);
      ...
      toast('Error processing billing');
    }
  };
  ...
  const filteredBilled = billedArray.filter(b => {
    let match = true;
    ...
    <div>
      <div style={{ display: 'flex', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
        <div style={styles.reportBox}>
          ...
          <div style={styles.reportValue}>{jobsArray.length}</div>
        </div>
        <div style={{ ...styles.reportBox, background: '#dbeafe' }}>
          <div style={styles.reportLabel}>✅ Total Billed</div>
          <div style={{ ...styles.reportValue, color: '#2563eb' }}>{billedArray.length}</div>
        </div>
        <div style={{ ...styles.reportBox, background: '#fef3c7' }}>
          <div style={styles.reportLabel}>💰 Total Revenue</div>
          <div style={{ ...styles.reportValue, color: '#d97706' }}>LKR {billedArray.reduce((sum, b) => sum + (b.netAmount || 0), 0).toFixed(2)}</div>
        </div>
        <div style={{ ...styles.reportBox, background: '#f0fdf4' }}>
          <div style={styles.reportLabel}>👥 Active Workers</div>
          <div style={{ ...styles.reportValue, color: '#16a34a' }}>{workersArray.filter(w => w.active !== false).length}</div>
        </div>
      </div>
      ...
      <GlassCard title="🔧 Repair Management">
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
          ...
          <select style={styles.input} value={filters.worker}
            ...
            {workersArray.filter(w => w.active !== false).map(w => (
          ...
          <>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table style={styles.table}>
                ...
                <tbody>
                  {!filteredJobs.length && <tr><td colSpan={7} style={styles.emptyTd}>No pending jobs</td></tr>}
                  {filteredJobs.map(j => (
                    <tr key={j.orderId}>
                ...
                <tbody>
                  ...
                  {filteredBilled.map(b => (
                    <tr key={b.id}>
                      ...
                      <td style={styles.td}><strong>LKR {(b.netAmount || 0).toFixed(2)}</strong></td>
      ...
      {showBillModal && selectedJob && (
        <Modal title={`💰 Bill Repair - ${selectedJob.orderId}`} onClose={() => setShowBillModal(false)}>
          <div style={{ maxHeight: '70vh', overflowY: 'auto', minWidth: 520 }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
              ...
              <button style={{ ...styles.btnPrimary, background: '#10b981' }} onClick={processBilling}>
                💰 Confirm & Bill
              </button>
            </div>
          </div>
